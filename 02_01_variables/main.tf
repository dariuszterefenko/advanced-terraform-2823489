...
resource "aws_instance" "nodejs1" {
  ...
  instance_type = var.environment_instance_settings[var.deploy_enviroment].instance_type
  subnet_id = aws_subnet.subnet1.id
  vpc_security_group_ids = [aws_security_group.sg-nodejs-instance.id]

  monitoring = var.environment_instance_settings[var.deploy_enviroment].monitoring

  tags = {Environment = var.environment_map[var.deploy_enviroment]}
